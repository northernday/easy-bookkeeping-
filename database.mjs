...
function createEmpty() {
  data = {
    ledgers: [{ id: 1, name: '默认账本', created_at: new Date().toISOString() }],
    categories: [
...
export async function registerDatabaseHandlers() {
  ...
  // 账本
  ipcMain.handle('db:getLedgers', () => data.ledgers);
  ...
  ipcMain.handle('db:createLedger', (_e, name) => {
    ...
    data.ledgers.push({ id, name, created_at: new Date().toISOString() });
    // 为新账本创建预设分类
